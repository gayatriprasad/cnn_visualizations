...
if __name__ == '__main__':
    ...
    VIS_TARGET_FOLDER = '/home/gp/Documents/results_VBP_random_train_final/'
    if not os.path.exists('/home/gp/Documents/results_VBP_random_train_final/'):
        os.makedirs('/home/gp/Documents/results_VBP_random_train_final/')
    ...
    for i in range(0, sample_amount):
        ...
        vbp_sample, label1 = dataset2[sample_id]
        # making copies for modification
        vbp_sample_copy = copy.deepcopy(vbp_sample)
        # Forward pass
        out = net(vbp_sample)
        print('out', out)
        # current prediction
        ...
        # Get gradients
        var_sample_1 = Variable(vbp_sample_copy, requires_grad=True)
        vanilla_grads = VBP.generate_gradients(var_sample_1, label1)
        # print('Vanilla_grads_np:', vanilla_grads.shape)
        split_grads_np1 = vanilla_grads[0]
        ...
        for j in range(0, 50):
            ...
            vbp_magnitude_list.append(np.linalg.norm(split_grads_np1[j]))
        ...
        for single_frame_replace_id in range(0, 50):
            vbp_sample_copy = copy.deepcopy(vbp_sample)
            vbp_sample_copy[0][single_frame_replace_id] = padding
            # Forwad pas with changed frame
            out = net(vbp_sample_copy)
        ...
        sorted_logit_list = [x for _, x in sorted(zip(vbp_magnitude_list, logit_difference_list))]
        # print()
    ...
    plt.savefig(VIS_TARGET_FOLDER + 'avg_logit_diff_list_all_train_samples_' + 'VBP_' +
                ".png", format="PNG")
    ...
    plt.savefig(VIS_TARGET_FOLDER + 'master_magnitude_all_train_samples_' + 'VBP_' +
                ".png",  format="PNG")
    ...
    plt.legend(loc='upper left')
    plt.savefig(VIS_TARGET_FOLDER + 'master_logit_diff_list_all_train_samples_' + 'VBP_' +
                ".png", format="PNG")
